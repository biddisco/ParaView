...
class VTKPVSERVERMANAGERRENDERING_EXPORT vtkSMPVRepresentationProxy: public vtkSMRepresentationProxy
  {
public:
  static vtkSMPVRepresentationProxy* New();vtkTypeMacro(vtkSMPVRepresentationProxy, vtkSMRepresentationProxy)
  ;
  ...
  static bool GetUsingScalarColoring(vtkSMProxy* proxy)
    {
    vtkSMPVRepresentationProxy* self = vtkSMPVRepresentationProxy::SafeDownCast(
        proxy);
  ...
  virtual bool RescaleTransferFunctionToCustomRange(double minScale,
      double maxScale, double minGradient = 0, double maxGradient = 1,
      bool extend = false);
  ...
  // If \c extend is true (false by default), the transfer function range will
  // only be extended as needed to fit the data range.
  virtual bool RescaleTransferFunctionToDataRange(const char* arrayname,
      int attribute_type, bool extend = false);

  // Description:
  // Rescales the gradient transfer functions to data range
  virtual bool RescaleGradientTransferFunctionToDataRange(const char* arrayname,
      int attribute_type, bool extend = false);
  ...
  static bool RescaleGradientTransferFunctionToDataRange(vtkSMProxy* proxy,
    ...
    vtkSMPVRepresentationProxy* self = vtkSMPVRepresentationProxy::SafeDownCast(
        proxy);
    return
        self ? self->RescaleGradientTransferFunctionToDataRange(extend) : false;
    }

  // Description:
  // Safely call RescaleGradientTransferFunctionToDataRange() after casting the proxy to
  // appropriate type.
  static bool RescaleGradientTransferFunctionsToCustomRange(vtkSMProxy* proxy,
    ...
    return
        self ?
            self->RescaleTransferFunctionToCustomRange(minScale, maxScale,
                minGradient, maxGradient, extend) :
            false;
    }
  ...
  static bool RescaleTransferFunctionToDataRange(vtkSMProxy* proxy,
    ...
    return
        self ?
            self->RescaleTransferFunctionToDataRange(arrayname, attribute_type,
                extend) :
            false;
    }
  ...
  static bool RescaleGradientTransferFunctionToDataRange(vtkSMProxy* proxy,
    ...
    return
        self ?
            self->RescaleGradientTransferFunctionToDataRange(arrayname,
                attribute_type, extend) :
            false;
    }
  ...
  // successful. \c field_association must be one of
  // vtkDataObject::AttributeTypes,
  virtual bool RescaleTransferFunctionToDataRangeOverTime(const char* arrayname,
      int attribute_type);
  ...
  static bool RescaleTransferFunctionToDataRangeOverTime(vtkSMProxy* proxy,
    ...
    return
        self ?
            self->RescaleTransferFunctionToDataRange(arrayname,
                attribute_type) :
            false;
    }
...
protected:
  ...
  virtual bool RescaleTransferFunctionToDataRange(vtkPVArrayInformation* info,
      bool extend = false);
  virtual bool RescaleGradientTransferFunctionToDataRange(
      vtkPVImageAccumulateInformation * gradientInfo,
