...
bool vtkSMPVRepresentationProxy::RescaleTransferFunctionToCustomRange(
  ...
  vtkSMProperty* gofProperty = this->GetProperty(
      "GradientLinearOpacityFunction");
  vtkSMProperty* gaussfProperty = this->GetProperty(
      "GradientGaussianOpacityFunction");
  ...
  if (!lutProperty && !gofProperty)
    {
    vtkWarningMacro(
        "No 'GradientLinearOpacityFunction' and 'LookupTable' found.");
    }
  if (!lutProperty && !gaussfProperty)
    {
    vtkWarningMacro(
        "No 'GradientGaussianOpacityFunction' and 'LookupTable' found.");
    }
  ...
  if (lut)
    ...
    vtkSMProxy* gaussf_lut = vtkSMPropertyHelper(lut,
    ...
    if (sgaussf_lut && sgaussf != sgaussf_lut)
      ...
      vtkSMTransferFunctionProxy::RescaleGaussianTransferFunction(sgaussf_lut,
          range, true);
      }
  ...
  if (sgaussf)
    {
    vtkSMTransferFunctionProxy::RescaleGaussianTransferFunction(sgaussf, range,
        true);
    }
  if (gof)
    {
...
int vtkSMPVRepresentationProxy::ReadXMLAttributes(vtkSMSessionProxyManager* pm,
  ...
  if (inputProperty)
    ...
    for (vtkStringSet::iterator iter = this->RepresentationSubProxies->begin();
      ...
      vtkSMProperty* subProperty =
          subProxy ? subProxy->GetProperty("Input") : NULL;
      if (subProperty)
        {
...
bool vtkSMPVRepresentationProxy::RescaleGradientTransferFunctionToDataRange(
  ...
  return this->RescaleGradientTransferFunctionToDataRange(gradientInfo, info,
      extend);
  }
...
//----------------------------------------------------------------------------
bool vtkSMPVRepresentationProxy::RescaleGradientTransferFunctionToDataRange(
    vtkPVImageAccumulateInformation * gradientInfo, vtkPVArrayInformation* info,
    bool extend)
  {
  if (!info)
    {
  ...
  vtkSMProperty* gofProperty = this->GetProperty(
      "GradientLinearOpacityFunction");
  vtkSMProperty* ggaussfProperty = this->GetProperty(
      "GradientGaussianOpacityFunction");
  vtkSMProperty* TDTransfProperty = this->GetProperty("TwoDTransferFunction");
