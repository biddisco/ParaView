...
void vtkPVImageAccumulateInformation::PrintSelf(ostream& os, vtkIndent indent)
  ...
  os << indent << "CollectGradientHistogram: " << this->CollectGradientHistogram
      << endl;
  os << indent << "CollectGradientRange: " << this->CollectGradientRange
      << endl;
  }
...
void vtkPVImageAccumulateInformation::CopyFromObject(vtkObject* obj)
  ...
  vtkImageVolumeRepresentation* volumerep =
      vtkImageVolumeRepresentation::SafeDownCast(
...
//----------------------------------------------------------------------------
void vtkPVImageAccumulateInformation::AddInformation(
...
//----------------------------------------------------------------------------
void vtkPVImageAccumulateInformation::CopyFromStream(
    const vtkClientServerStream* stream)
  {
  int N = 0;
  if (!stream->GetArgument(0, 0, &this->CollectGradientHistogram)
      || !stream->GetArgument(0, 1, &this->CollectGradientRange))
    {
  ...
  if (this->CollectGradientRange)
    {
    if (!stream->GetArgument(0, N, &(this->GradientRange[0]))
        || !stream->GetArgument(0, N + 1, &(this->GradientRange[1])))
      {
      vtkErrorMacro("Error getting gradient range from message.");
      return;
      }
    }
  if (!stream->GetArgument(0, N + 2, &(this->CurrentGradientRange[0]))
      || !stream->GetArgument(0, N + 3, &(this->CurrentGradientRange[1])))
    {
    vtkErrorMacro("Error getting gradient range from message.");
    return;
    }
...
//-----------------------------------------------------------------------------
void vtkPVImageAccumulateInformation::CopyToStream(
