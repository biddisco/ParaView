...
void pqTwoDTransferFunction::setRange(double xmin, double xmax, double ymin,
    double ymax)
  {
  ...
  if (dnewx > 0)
    ...
    for (int cc = 0; cc < controlPoints.size(); cc += 4)
      {
      controlPoints[cc] = xscale
          * (controlPoints[cc].toDouble() - current_x_range.first) + xmin;
      cc += 2;
  ...
  if (dnewy > 0)
    ...
    for (int cc = 0; cc < controlPoints.size(); cc += 4)
      {
      controlPoints[cc] = yscale
          * (controlPoints[cc].toDouble() - current_y_range.first) + ymin;
      cc += 2;
