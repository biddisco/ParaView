...
void pqHistogramWidget::SetData(std::vector<bool>* histogramEnable,
    std::vector<int>* histogra, int histogramSiz, bool logSc,
    float enabledBarsFrac)
  {
...
void pqHistogramWidget::createPixmap()
  ...
  for (int i = 0; i < width; i++)
    ...
    if (this->histogram->at(i) > this->currentUnEnabledMax
        && !this->histogramEnabled->at(i))
  ...
  float scale;
  if (this->logScale)
    scale = float(height - this->enabledBarsHeight)
        / float(log10((double) (max)));
  else
  ...
  for (int i = 0; i < width; i++)
    ...
    QRgb color =
        this->histogramEnabled->at(i) ? qRgb(200, 0, 0) : qRgb(100, 0, 0);
...
int pqHistogramWidget::getTopBinPixel(int bin, float scale)
  ...
  if (!this->histogramEnabled->at(bin)
      && this->histogram->at(bin) > this->currentMax)
    {
    if (this->logScale)
    ...
    if (this->logScale)
      finalheight = int(
          scale
              * std::max((double) log10(float(this->histogram->at(bin))), 0.0));
    else
...
void pqHistogramWidget::mousePressEvent(QMouseEvent *e)
  ...
  if ((this->histogramEnabled->at(selectedBin)
      && this->histogram->at(selectedBin) > this->currentMax)
      || (!this->histogramEnabled->at(selectedBin)
          && this->histogram->at(selectedBin) == this->currentMax)) //TBD fix bug (what if 2 histograms the same
    updateAllBinColumns();
  else
