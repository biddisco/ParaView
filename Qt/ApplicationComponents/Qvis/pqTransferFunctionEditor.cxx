...
//-----------------------------------------------------------------------------
pqTransferFunctionEditor::pqTransferFunctionEditor(QWidget *parent,
    Qt::WindowFlags f) :
    QFrame(parent)
  {
  ...
  this->connect(this->Internals->TransferFunctionMode,
      SIGNAL(currentIndexChanged(int)), this, SLOT(onTFModeChanged(int)),
      Qt::QueuedConnection);

  this->connect(this->Internals->OpacityLevel, SIGNAL(valueChanged(int)), this,
      SLOT(onOpacityLevelChanged(int)), Qt::QueuedConnection);

  this->connect(this->Internals->TransferFunction,
      SIGNAL(activeRegionChanged(int)), this, SLOT(onActiveRegionChanged(int)),
      Qt::QueuedConnection);
...
//-----------------------------------------------------------------------------
void pqTransferFunctionEditor::initialize(vtkTwoDTransferFunction* function,
  ...
  if (function || stc)
    ...
    this->connect(this->Internals->TransferFunction,
        SIGNAL(controlPointsModified()), this, SIGNAL(controlPointsModified()));
    }
...
QList<QVariant> pqTransferFunctionEditor::regionControlPoints()
  {
  QList<QVariant> list;
  for (int i = 0; i < this->Internals->TransferFunction->getNumberOfRegions();
      ++i)
    {
    float g[REGION_VARS];
    this->Internals->TransferFunction->getRegion(i, &g[0], &g[1], &g[2], &g[3],
        &g[4], &g[5]);
...
//-----------------------------------------------------------------------------
void pqTransferFunctionEditor::setRegionControlPoints(
...
void pqTransferFunctionEditor::onActiveRegionChanged(int region)
  ...
  if (this->Internals->TransferFunction->getActiveRegionMaximum() > -1)
    ...
    this->Internals->OpacityLevel->setValue(
        val * this->Internals->OpacityLevel->maximum());
    this->Internals->OpacityLevel->update();
    this->Internals->TransferFunctionMode->setCurrentIndex(m);
    }
  }

void pqTransferFunctionEditor::generateHistogramBackground(int width,
    int height, std::vector<int> &array, std::vector<bool> &enabledBins,
    bool logScale)
  {
  this->Internals->TransferFunction->generateHistogramBackground(width, height,
      array, enabledBins, logScale);
  }
