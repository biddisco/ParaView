...
Qvis2DTransferFunctionWidget::Qvis2DTransferFunctionWidget(
    QWidget *parentObject, const char *name) :
...
Qvis2DTransferFunctionWidget::~Qvis2DTransferFunctionWidget()
  ...
  if (this->UnderlayColourPixmap)
    delete this->UnderlayColourPixmap;
  }
...
void Qvis2DTransferFunctionWidget::initialize(vtkTwoDTransferFunction* function,
  ...
  if (stc)
    ...
    this->VTKConnect->Connect(colortransferfunction, vtkCommand::ModifiedEvent,
...
int Qvis2DTransferFunctionWidget::addRegion(double _x, double _y, double _w,
    double _h, TransferFnMode mode, double max)
  {
  ...
  return this->transferFunction->AddRegion(functionX, functionY, functionW,
      functionH, mode, max);

  }

Qvis2DTransferFunctionWidget::Region Qvis2DTransferFunctionWidget::getRegion(
    int index)
  {
  double xshift = this->transferFunction->GetRangeAtIndex(0);
  double yshift = this->transferFunction->GetRangeAtIndex(2);
  double regionX = this->transferFunction->getValue(index,
      vtkTwoDTransferFunction::REGION_X) / this->transferFunction->getXRange()
      - xshift / this->transferFunction->getXRange();
  double regionY = this->transferFunction->getValue(index,
      vtkTwoDTransferFunction::REGION_Y) / this->transferFunction->getYRange()
      - yshift / this->transferFunction->getYRange();
  double regionW = this->transferFunction->getValue(index,
      vtkTwoDTransferFunction::REGION_W) / this->transferFunction->getXRange();
  double regionH = this->transferFunction->getValue(index,
      vtkTwoDTransferFunction::REGION_H) / this->transferFunction->getYRange();
  double max = this->transferFunction->getValue(index,
      vtkTwoDTransferFunction::REGION_MAX);
...
double Qvis2DTransferFunctionWidget::getRegionValue(int index,
    vtkTwoDTransferFunction::regionvalue value)
  {
  double xshift = this->transferFunction->GetRangeAtIndex(0);
  double yshift = this->transferFunction->GetRangeAtIndex(2);
  if (value == vtkTwoDTransferFunction::REGION_MODE)
    ...
    return this->transferFunction->getValue(index,
        vtkTwoDTransferFunction::REGION_X) / this->transferFunction->getXRange()
        - xshift / this->transferFunction->getXRange(); ///this->transferFunction->getXRange()*double(this->contentsRect().width());
  else if (value == vtkTwoDTransferFunction::REGION_Y)
    return this->transferFunction->getValue(index,
        vtkTwoDTransferFunction::REGION_Y) / this->transferFunction->getYRange()
        - yshift / this->transferFunction->getYRange(); ///this->transferFunction->getYRange()*double(this->contentsRect().height());
  else if (value == vtkTwoDTransferFunction::REGION_W)
    return this->transferFunction->getValue(index,
        vtkTwoDTransferFunction::REGION_W) / this->transferFunction->getXRange(); ///this->transferFunction->getXRange()*double(this->contentsRect().width());
  else if (value == vtkTwoDTransferFunction::REGION_H)
    return this->transferFunction->getValue(index,
        vtkTwoDTransferFunction::REGION_H) / this->transferFunction->getYRange(); ///this->transferFunction->getYRange()*double(this->contentsRect().height());
...
void Qvis2DTransferFunctionWidget::setRegion(int index,
    Qvis2DTransferFunctionWidget::Region &reg)
  {
  double xshift = this->transferFunction->GetRangeAtIndex(0);
  double yshift = this->transferFunction->GetRangeAtIndex(2);
  double values[5];
...
//TBD convert space
void Qvis2DTransferFunctionWidget::setRegionValue(int index, double value,
  ...
  if (v == vtkTwoDTransferFunction::REGION_X)
    this->transferFunction->SetRegionValue(index,
        value * this->transferFunction->getXRange() + xshift, v);
  else if (v == vtkTwoDTransferFunction::REGION_Y)
    this->transferFunction->SetRegionValue(index,
        value * this->transferFunction->getYRange() + yshift, v);
  else if (v == vtkTwoDTransferFunction::REGION_W)
    this->transferFunction->SetRegionValue(index,
        value * this->transferFunction->getXRange(), v);
  else if (v == vtkTwoDTransferFunction::REGION_H)
    this->transferFunction->SetRegionValue(index,
        value * this->transferFunction->getYRange(), v);
    ...
    this->transferFunction->SetRegionMode(index,
        TransferFnMode(vtkMath::Round(value)));
  }
...
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::setBackgroundGradientData(int x, int y,
    int *data)
  {
...
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::setUnderlayColourData(int N, int c,
  ...
  if (N > 0 && data)
    ...
    for (int i = 0; i < N; i++)
      {
      image.setPixel(i, 0,
          qRgb(data[c * i + 0], data[c * i + 1], data[c * i + 2]));
      }
    if (this->UnderlayColourPixmap)
      delete this->UnderlayColourPixmap;
...
void Qvis2DTransferFunctionWidget::drawColourBars(QPainter &painter)
  ...
  for (int p = 0; p < nregion; p++)
    {
    double x1 = Val2x(pix,
        getRegionValue(p, vtkTwoDTransferFunction::REGION_X));
    double y1 = Val2y(pix,
        getRegionValue(p, vtkTwoDTransferFunction::REGION_Y));
    double x2 = Val2x(pix,
        getRegionValue(p, vtkTwoDTransferFunction::REGION_X)
            + getRegionValue(p, vtkTwoDTransferFunction::REGION_W));
    double y2 = Val2y(pix,
        getRegionValue(p, vtkTwoDTransferFunction::REGION_Y)
            + getRegionValue(p, vtkTwoDTransferFunction::REGION_H));
    double wx = x2 - x1;
    double wy = y1 - y2;
    double mx = (double) this->UnderlayColourPixmap->width()
    ...
    for (double i = 0; i < wx; i++)
      ...
      for (double j = 0; j < wy; j++)
        {
        switch ((TransferFnMode) int(
            getRegionValue(p, vtkTwoDTransferFunction::REGION_MODE) + 0.1))
          {
        case Uniform:
          intensity = 1.0;
          break;
        ...
        intensity = getRegionValue(p, vtkTwoDTransferFunction::REGION_MAX)
            * intensity;
...
void Qvis2DTransferFunctionWidget::drawControlPoints(QPainter &painter)
  ...
  for (int p = 0; p < nregion; p++)
    ...
    pts.setPoints(4, x1 - 2, y1 - 2, x1 - 2, y1 + 2, x1 + 2, y1 + 2, x1 + 2,
        y1 - 2);
    ...
    if (highlitRegion == p && (movingMode == modeC1 || movingMode == modeAll))
      ...
      painter.setPen(bluepen);
    pts.setPoints(4, x1 - 2, y2 - 2, x1 - 2, y2 + 2, x1 + 2, y2 + 2, x1 + 2,
        y2 - 2);
    painter.drawPolygon(pts);
    ...
    if (highlitRegion == p && (movingMode == modeC2 || movingMode == modeAll))
      ...
      painter.setPen(bluepen);
    pts.setPoints(4, x2 - 2, y2 - 2, x2 - 2, y2 + 2, x2 + 2, y2 + 2, x2 + 2,
        y2 - 2);
    painter.drawPolygon(pts);
    ...
    if (highlitRegion == p && (movingMode == modeC3 || movingMode == modeAll))
      ...
      painter.setPen(bluepen);
    pts.setPoints(4, x2 - 2, y1 - 2, x2 - 2, y1 + 2, x2 + 2, y1 + 2, x2 + 2,
        y1 - 2);
    painter.drawPolygon(pts);
    }
  }
...
void Qvis2DTransferFunctionWidget::createScalarColorBackground()
  ...
  this->transferFunction->GetRange(currentRangeMinX, currentRangeMaxX,
      currentRangeMinY, currentRangeMaxY);

  const unsigned char * c = this->colortransferfunction->GetTable(
      currentRangeMinX, currentRangeMaxX, this->contentsRect().width());
  ...
  this->transferFunction->GetTable(currentRangeMinX, currentRangeMaxX,
      currentRangeMinY, currentRangeMaxY, this->contentsRect().width(),
      this->contentsRect().height(), this->backgroundOpacityValues);

  QImage image(
      QSize(this->contentsRect().width(), this->contentsRect().height()),
      QImage::Format_RGB32);
  image.fill(Qt::white);

  if (!this->histogramBackground.isNull())
    {
    QImage scaledhisto = this->histogramBackground->scaled(
    ...
    for (int i = 0; i < this->contentsRect().width(); i++)
      ...
      for (int j = 0; j < this->contentsRect().height(); j++)
        {
        int r = (int) c[i * 3]
            * this->backgroundOpacityValues[i * this->contentsRect().height()
                + j]
            + 255
                * (1
                    - this->backgroundOpacityValues[i
                        * this->contentsRect().height() + j]);
        r = int(
            double(255 - qRed(scaledhisto.pixel(i, j))) / 255.0 * double(r));
            ...
            + 255
                * (1
                    - this->backgroundOpacityValues[i
                        * this->contentsRect().height() + j]);
        g = int(
            double(255 - qGreen(scaledhisto.pixel(i, j))) / 255.0 * double(g));
            ...
            + 255
                * (1
                    - this->backgroundOpacityValues[i
                        * this->contentsRect().height() + j]);
        b = int(
            double(255 - qBlue(scaledhisto.pixel(i, j))) / 255.0 * double(b));
  ...
  QPixmap* background = new QPixmap(QPixmap::fromImage(image));
...
void Qvis2DTransferFunctionWidget::mousePressEvent(QMouseEvent *e)
  ...
  if (e->button() == Qt::RightButton)
    ...
    if (!findRegionControlPoint(_x, _y, &testregion, &movingMode))
      ...
      movingMode = modeC2;
      if (maximumNumberOfRegions == -1
          || getNumberOfRegions() < maximumNumberOfRegions)
        {
...
void Qvis2DTransferFunctionWidget::mouseMoveEvent(QMouseEvent *e)
  ...
  case modeC0:
    ...
    setRegionValue(r, getRegionValue(r, vtkTwoDTransferFunction::REGION_W) - dx,
        vtkTwoDTransferFunction::REGION_W);
    setRegionValue(r, getRegionValue(r, vtkTwoDTransferFunction::REGION_H) - dy,
        vtkTwoDTransferFunction::REGION_H);
    break;
  case modeC1:
    dx = nx - getRegionValue(r, vtkTwoDTransferFunction::REGION_X);
    dy = ny
        - (getRegionValue(r, vtkTwoDTransferFunction::REGION_Y)
            + getRegionValue(r, vtkTwoDTransferFunction::REGION_H));
    setRegionValue(r, nx, vtkTwoDTransferFunction::REGION_X);
    setRegionValue(r, getRegionValue(r, vtkTwoDTransferFunction::REGION_W) - dx,
        vtkTwoDTransferFunction::REGION_W);
    setRegionValue(r, getRegionValue(r, vtkTwoDTransferFunction::REGION_H) + dy,
  ...
  case modeC2:
    dx = nx
        - (getRegionValue(r, vtkTwoDTransferFunction::REGION_X)
            + getRegionValue(r, vtkTwoDTransferFunction::REGION_W));
    dy = ny
        - (getRegionValue(r, vtkTwoDTransferFunction::REGION_Y)
            + getRegionValue(r, vtkTwoDTransferFunction::REGION_H));
    setRegionValue(r, getRegionValue(r, vtkTwoDTransferFunction::REGION_W) + dx,
  ...
  case modeAll:
    dx = nx - lx;
    dy = ny - ly;
    if ((getRegionValue(r, vtkTwoDTransferFunction::REGION_X)
        + getRegionValue(r, vtkTwoDTransferFunction::REGION_W) + dx) < xmax
        && (getRegionValue(r, vtkTwoDTransferFunction::REGION_Y)
            + getRegionValue(r, vtkTwoDTransferFunction::REGION_H) + dy) < ymax
        && (getRegionValue(r, vtkTwoDTransferFunction::REGION_X) + dx) >= 0
        && (getRegionValue(r, vtkTwoDTransferFunction::REGION_Y) + dy) >= 0)
      {
      setRegionValue(r,
          getRegionValue(r, vtkTwoDTransferFunction::REGION_X) + dx,
          vtkTwoDTransferFunction::REGION_X);
      setRegionValue(r,
          getRegionValue(r, vtkTwoDTransferFunction::REGION_Y) + dy,
  ...
  if (getRegionValue(r, vtkTwoDTransferFunction::REGION_W) < 0)
    ...
    setRegionValue(r,
        getRegionValue(r, vtkTwoDTransferFunction::REGION_X)
            + getRegionValue(r, vtkTwoDTransferFunction::REGION_W),
        vtkTwoDTransferFunction::REGION_X);
    setRegionValue(r, -getRegionValue(r, vtkTwoDTransferFunction::REGION_W),
        vtkTwoDTransferFunction::REGION_W);
    if (movingMode == modeC0)
      movingMode = modeC3;
    else if (movingMode == modeC1)
      movingMode = modeC2;
    else if (movingMode == modeC2)
      movingMode = modeC1;
    else if (movingMode == modeC3)
      movingMode = modeC0;
    }
  if (getRegionValue(r, vtkTwoDTransferFunction::REGION_H) < 0)
    ...
    setRegionValue(r,
        getRegionValue(r, vtkTwoDTransferFunction::REGION_Y)
            + getRegionValue(r, vtkTwoDTransferFunction::REGION_H),
        vtkTwoDTransferFunction::REGION_Y);
    setRegionValue(r, -getRegionValue(r, vtkTwoDTransferFunction::REGION_H),
        vtkTwoDTransferFunction::REGION_H);
    if (movingMode == modeC0)
      movingMode = modeC1;
    else if (movingMode == modeC1)
...
void Qvis2DTransferFunctionWidget::setActiveRegionMode(int index)
  ...
  if (this->activeRegion != -1)
    {
    setRegionValue(this->activeRegion, TransferFnMode(index),
        vtkTwoDTransferFunction::REGION_MODE);
...
int Qvis2DTransferFunctionWidget::getActiveRegionMode()
  ...
  if (this->activeRegion == -1)
    return -1;
  return getRegionValue(this->activeRegion,
      vtkTwoDTransferFunction::REGION_MODE);
  }
//---------------------------------------------------------------------------
...
float Qvis2DTransferFunctionWidget::getActiveRegionMaximum()
  ...
  if (this->activeRegion < 0
      || this->activeRegion > this->transferFunction->GetSize())
    return -1;
  return this->transferFunction->getValue(this->activeRegion,
      vtkTwoDTransferFunction::REGION_MAX);
  }
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::addRegion(float x, float y, float w, float h,
    float mo, float mx)
  {
  addRegion(double(x), double(y), double(w), double(h),
      (TransferFnMode) int(mo), double(mx));
  }
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::removeRegion(int n)
  {
...
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::setMaximumNumberOfRegions(int n)
  {
  this->maximumNumberOfRegions = n;
  }
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::setMinimumNumberOfRegions(int n)
  {
...
bool Qvis2DTransferFunctionWidget::InRegion(int index, float *pt)
  {
  if (pt[0] < getRegionValue(index, vtkTwoDTransferFunction::REGION_X))
    return false;
  if (pt[0]
      > getRegionValue(index, vtkTwoDTransferFunction::REGION_X)
          + getRegionValue(index, vtkTwoDTransferFunction::REGION_W))
    return false;
  if (pt[1] < getRegionValue(index, vtkTwoDTransferFunction::REGION_Y))
    return false;
  if (pt[1]
      > getRegionValue(index, vtkTwoDTransferFunction::REGION_Y)
          + getRegionValue(index, vtkTwoDTransferFunction::REGION_H))
    return false;
  return true;
  }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
bool Qvis2DTransferFunctionWidget::findRegionControlPoint(int _x, int _y,
    int *newregion, SelectMode *newmode)
  {
...
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::getRegion(int i, float *x, float *y,
    float *w, float *h, float *mo, float *mx)
  {
...
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::setRegion(int i, float *x, float *y,
    float *w, float *h, float *mo, float *mx)
  {
...
void Qvis2DTransferFunctionWidget::setAllRegions(int n, float *regiondata)
  ...
  for (int i = 0; i < n; i++)
    ...
    addRegion(regiondata[i * 5 + 0], regiondata[i * 5 + 1],
        regiondata[i * 5 + 2], regiondata[i * 5 + 3], regiondata[i * 5 + 4],
        regiondata[i * 5 + 5]);
    }
  this->update();
  }
//-----------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::generateHistogramBackground(int width,
    int height, std::vector<int> &array, std::vector<bool> &enabledBins,
    bool logScale)
  {
  //find max
  int size = array.size();
   ...
   */
  histogramBackground = QSharedPointer<QImage>(
  ...
  for (int i = 0; i < width; i++)
    ...
    for (int j = 0; j < height; j++)
      ...
      if (enabledBins[i * height + j])
        {
        float v = float(array[i * height + j]);
        if (logScale && v >= 1)
          v = log10(v);
        v = v / maxf;
        histogramBackground->setPixel(i, j,
            qRgb(255.0f * v, 255.0f * v, 255.0f * v));
        }
      else
        {
